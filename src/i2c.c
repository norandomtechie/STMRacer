...
//      char msg_new[] = "abcdefg";
        ...
        while(1) {
         if (i2c_senddata(0x50, newdata, sizeof newdata) < 0) {
         I2C1->ICR |= I2C_ICR_NACKCF;
         I2C1->ICR |= I2C_ICR_STOPCF;
         } else
         break;
...
char* display_eeprom(int addr, int len) {
    ...
    char *buf = malloc ((255+16) * sizeof (char));
    buf[0] = (addr >> 8) & 0x0f;
    buf[1] = addr & 0x0ff;
...
int main(void)
{
    ...
    int b = 500;
    int c = 400;
    char astr[3] = {a / 100 + '0', a / 10 + '0', a % 10 + '0'};
    char bstr[3] = {b / 100 + '0', b / 10 + '0', b % 10 + '0'};
    char cstr[3] = {c / 100 + '0', c / 10 + '0', c % 10 + '0'};

    write_eeprom(0,  astr);
    write_eeprom(32, bstr);
    write_eeprom(64, cstr);
    nano_wait (1000000000);
    int ain = atoi (display_eeprom(0,255));
    int bin = atoi (display_eeprom(32,255));
    int cin = atoi (display_eeprom(64,255));
    ...
    else if (score < ain && score > bin)
    {
      bin = score;
    }
    else if (score < bin && score > cin)
    {
      cin = score;
    }

    char aout[3] = {ain / 100 + '0', ain / 10 + '0', ain % 10 + '0'};
    char bout[3] = {bin / 100 + '0', bin / 10 + '0', bin % 10 + '0'};
    char cout[3] = {cin / 100 + '0', cin / 10 + '0', cin % 10 + '0'};

    write_eeprom(0,  aout);
    write_eeprom(32, bout);
    write_eeprom(64, cout);

    nano_wait (1000000000);
    ain = display_eeprom(0,255);
    bin = display_eeprom(32,255);
    cin = display_eeprom(64,255);
